...
class Game:
    def __init__(self, screen):
        self.screen = screen
        self.bg_image = pygame.image.load(os.path.join(ap, "images", "background.png")).convert()
        self.starship = Starship(os.path.join(ap, "images", "starship.png"))
        self.bullets = []
        self.asteroids = []
        self.booster = None
    ...
    def shield(self, mode):
        if mode == "activate":
            self.starship = Starship(os.path.join(ap, "images", "Starship_with_shield.png"), self.starship.pos)
        elif mode == "deactivate":
            self.starship = Starship(os.path.join(ap, "images", "starship.png"), self.starship.pos)
...
class Starship:
    def __init__(self, original_image=os.path.join(ap, "images", "starship.png"),
                       pos=arr([SCREEN_SIZE[0] / 2, SCREEN_SIZE[1] / 2])):
...
class Bullet:
    original_image = pygame.image.load(os.path.join(ap, "images", "bullet.png"))
...
class Asteroid:
    variants = [("small", pygame.image.load(os.path.join(ap, "images", "ast1_small.png")), (10, 10)),
    ...
    def _left_pos(self):
        return arr((random.uniform(-50, 0),
                        random.uniform(0, SCREEN_SIZE[1])))

    def _right_pos(self):
        return arr((random.uniform(SCREEN_SIZE[0], SCREEN_SIZE[0] + 50),
                        random.uniform(0, SCREEN_SIZE[1])))

    def _top_pos(self):
        return arr((random.uniform(0, SCREEN_SIZE[0]),
                        random.uniform(-50, 0)))
    ...
    def _speed_offset(self):
        offset = self.speed * 0.5 + un(-0.5, 0.5, 2)
        return offset

    def _pos_offset(self):
        offset = self.pos + un(-10, 10, 2)
        return offset
...
class Booster:
    def __init__(self, boost_type):
        ...
        self.image = pygame.image.load(os.path.join(ap, "images", boost_type + ".png"))
        self.pos = arr([random.randint(100, SCREEN_SIZE[0] - 100),
